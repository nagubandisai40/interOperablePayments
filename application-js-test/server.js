...
app.post('/test_view', (req, res) => {
    ...
    axios.get('http://localhost:5000/balance', { headers: { 'rippleId': id } }).then(response => {
...
app.post('/balanceTransfer', (req, res) => {
    ...
    axios.post('http://localhost:5000/transfer', { 'senderId': senderId, 'rcvrId': rcvrId, 'scrt': secret, 'value': value }).then(response => {
        console.log(response);
...
app.post('/createAsset', userValidations.isLoggedIn, (req, res) => {
    ...
    helper.createAsset(userName, id, issueId, issueName, owner, value, cat, assetName).then(value => {
        console.log(value)
        res.json(value)
    }).catch(err => {
        console.log(err);
...
app.post('/transferAsset', userValidations.isLoggedIn, (req, res) => {
    ...
    axios.post('http://localhost:5000/transfer', { 'senderId': senderAddr, 'rcvrId': rcvrAddr, 'scrt': scrt, 'value': val }).then(response => {
        console.log(response);
        res.status(200).send(response['data']);
