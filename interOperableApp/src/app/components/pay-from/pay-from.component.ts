...
export class PayFromComponent implements OnInit {
  constructor(
    private service: CorefunctionsService,
  ...
  ngOnInit(): void {
    ...
    this.senderAddr = JSON.parse(localStorage.getItem("access-token"))[
      "message"
    ]["rippleId"];
    this.userName = JSON.parse(localStorage.getItem("access-token")).message[
      "userName"
    ];
    ...
    this.service.getOwnerId(ownerId).subscribe(
      (res) => {
        // console.log(res)
